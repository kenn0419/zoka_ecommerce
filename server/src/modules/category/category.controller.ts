...
export class CategoryController {
  ...
  @Serialize(CategoryResponse, `Create new category successfully!`)
  create(
  ...
  @Get()
  @Serialize(CategoryResponse, `Find all categories successfully!`)
  findAllCategories(
  ...
  @Get('tree')
  @Serialize(CategoryResponse, `Find all tree successfully!`)
  getTree() {
    return this.categoryService.getTree();
  }

  @Get(':slug/children')
  @Serialize(CategoryResponse, `Find category's children successfully!`)
  getChildren(@Param('slug') slug: string) {
    return this.categoryService.getChildren(slug);
  }

  @Get(':slug')
  @Serialize(CategoryResponse, `Find category successfully!`)
  findOne(@Param('slug') slug: string) {
    return this.categoryService.findOne(slug);
  }

  @Patch(':slug')
  @Serialize(CategoryResponse, `Update category successfully!`)
  update(@Param('slug') slug: string, @Body() dto: UpdateCategoryDto) {
    return this.categoryService.update(slug, dto);
  }

  @Patch(':slug/activate')
  @Serialize(CategoryResponse, `Active category successfully!`)
  activate(@Param('slug') slug: string) {
    return this.categoryService.activate(slug);
  }

  @Patch(':slug/deactivate')
  @Serialize(CategoryResponse, `Deactive category successfully!`)
